...
public class BivariateNormalModel implements PlotterContract.Model {

    // Compute conditional probability P(X > a | Y > b)
    public static double computeConditionalProbability(
            double a, double b,
    ...
    public static double computeJointProbability(
            double a, double b,
            double meanX, double meanY,
            double sigmaX, double sigmaY,
            double rho
        ...
        double upperX = meanX + 5 * sigmaX, upperY = meanY + 5 * sigmaY;
        double dx = (upperX - a) / stepsX;
        double dy = (upperY - b) / stepsY;
    ...
    public static double computeMarginalProbabilityY(
            double b,
            double mean,
            double sigma
    ) {
